...
class yolo_loss:
    ...
    def __call__(self, input, target):
        ...
        # �?Predictor为基准，每个Predictor都有且仅有一�?需要负责的Target（前提是Predictor所在Grid Cell有Target�?心位于�?�）
        # x, y, w, h, c
        match = []
        conf = []
        ...
        # 计算每一个样本的损失，这里。按照论文应该是使用均方误差计算的
        for i in range(self.batch_size):
    ...
    #这个方法将目标标签从原始的 xmin, ymin, xmax, ymax, class 格式转换为适合YOLO网格单元的格式。好像和神经网络没什么关系。
    ...
    # 计算损失
    def compute_loss(self, input, target, match, conf,like_weight=20):
        ...
        #通过 sigmoid 函数将边界框的相关信息映射到0到1之间，适用于YOLO的输出
        ...
        for i in range(self.s ** 2):
            ...
            if target[i] is None:
                ...
                # 加入对点赞的手势的识别权重
                if self.isHand(input_class[i]):
                    l *= like_weight

            #累加损失：将每个网格单元的各类损失累加到对应的损失张量中。
            loss[i] = torch.sum(l)
...
def isHand(self, input_class):
        # 类别索引 0 是点赞的手势
        Hand = 0
        return torch.argmax(input_class) == Hand
